...
prepare_release_packages "$(<"$TEST_ROOT/candidates.ndjson")" "$manifest" "$recipe_key"
if [[ -e $OUT_DIR ]]; then
...
select_release_candidates "$TEST_ROOT/releases.ndjson" "$TEST_ROOT/candidates.ndjson"
enforce_pages_size_limit "$TEST_ROOT/candidates.ndjson" "$TEST_ROOT/selected.ndjson" "$manifest" "$recipe_key"
...
# A fresh publisher run must reuse cached packages, including the older release
# that the previous artifact-size check evicted from publication.
github_api_get() {
	jq --slurp 'map(. + {draft: false, prerelease: false})' "$TEST_ROOT/releases.ndjson"
}
curl() {
	fail 'cached repository run attempted an upstream download'
}
build_deb_from_asset() {
	fail 'cached repository run attempted to rebuild a package'
}
rm --force "$TEST_ROOT/first-measurement"
main
if [[ $(jq --raw-output '.newest_release.release_id' "$OUT_DIR/releases.json") != 2 ]]; then
	fail 'cached repository run did not retain the newest release'
fi
if [[ ! -f $package_path ]]; then
	fail 'eviction removed the older package from the extra cache budget'
fi
